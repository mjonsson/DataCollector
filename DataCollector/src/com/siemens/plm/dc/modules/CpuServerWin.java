...
public class CpuServerWin extends Module
{
	...
	protected final void rrdCreate()
		...
		if (!this.rrdProcessor.exists())
		{
			...
			rrdDef.addDatasource("procusertime", DsType.GAUGE, this.sampleInterval * 2, 0.0D, 100.0D);
			rrdDef.addDatasource("procprivtime", DsType.GAUGE, this.sampleInterval * 2, 0.0D, 100.0D);
			rrdDef.addDatasource("procinttime", DsType.GAUGE, this.sampleInterval * 2, 0.0D, 100.0D);
			rrdDef.addDatasource("queuelen", DsType.GAUGE, this.sampleInterval * 2, 0.0D, (0.0D / 0.0D));
	...
	{
		super.addSamples(this.rrdProcessor, this.procUserTime, this.procPrivTime, this.procIntTime, this.procQueueLen);
	}
	...
	protected final void buildGraph() throws Exception
	{
		...
		graphDef.datasource("procusertimeave", this.rrdProcessor.getAbsolutePath(), "procusertime", ConsolFun.AVERAGE);
		graphDef.datasource("procprivtimeave", this.rrdProcessor.getAbsolutePath(), "procprivtime", ConsolFun.AVERAGE);
		graphDef.datasource("procinttimeave", this.rrdProcessor.getAbsolutePath(), "procinttime", ConsolFun.AVERAGE);
		graphDef.datasource("queuelenave", this.rrdProcessor.getAbsolutePath(), "queuelen", ConsolFun.AVERAGE);
		graphDef.datasource("procusertimemax", this.rrdProcessor.getAbsolutePath(), "procusertime", ConsolFun.MAX);
		graphDef.datasource("procprivtimemax", this.rrdProcessor.getAbsolutePath(), "procprivtime", ConsolFun.MAX);
		graphDef.datasource("procinttimemax", this.rrdProcessor.getAbsolutePath(), "procinttime", ConsolFun.MAX);
		graphDef.datasource("queuelenmax", this.rrdProcessor.getAbsolutePath(), "queuelen", ConsolFun.MAX);
		...
		graphDef.line("queuelenave", new Color(0, 0, 0), "Processor Queue", 1.5F);
		graphDef.setMinValue(0.0D);
		graphDef.setMaxValue(100.0D);
		graphDef.comment("\\l");
		graphDef.comment("\\l");
		graphDef.gprint("procusertimeave", ConsolFun.AVERAGE, "User Avg: %3.0f %%");
		graphDef.gprint("procprivtimeave", ConsolFun.AVERAGE, "Sys Avg: %3.0f %%");
		graphDef.gprint("procinttimeave", ConsolFun.AVERAGE, "Inter. Avg: %3.0f %%");
	...
	public final boolean initialize() throws Exception
	{
		...
		this.rrdProcessor = new File(this.rrdFilePath + File.separator + this.id + "_processor.rrd");
		this.graphProcessor = new File(this.graphFilePath + File.separator + this.id + "_processor");
		//this.graphQueue = new File(this.graphFilePath + File.separator + this.id + "_queue");
